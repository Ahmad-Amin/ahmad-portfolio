...
export async function NpmRow() {
  ...
  return (
    <Panel as="li">
      ...
      {packages === null && <p className="mt-6 text-sm text-muted">Package data is temporarily unavailable.</p>}

      {packages !== null && packages.length === 0 && <p className="mt-6 text-sm text-muted">No published packages yet.</p>}
            ...
            <li key={pkg.name}>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
                <p className="font-mono text-base font-semibold text-foreground">
                  {pkg.name}
                  <span className="ml-2 font-mono text-sm font-normal tabular-nums text-muted">v{pkg.version}</span>
                </p>
              ...
              {pkg.description && <p className="mt-2 leading-relaxed text-muted">{pkg.description}</p>}
